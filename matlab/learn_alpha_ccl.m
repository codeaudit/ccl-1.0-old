...
%         model.nbStates = model.dim_b;
%         model          = init_GMM_kmeans(X, model);
%         model          = EM_GMM(X, model);
% %         model.phi      = @(x) phi_gaussian_rbf_GMM ( x, model);   % gaussian kernel basis function    
%         centres        = model.Mu;
%         variance       = real(mean(mean(sqrt(distances(centres, centres))))^2) ;
%         model.phi      = @(x) phi_gaussian_rbf ( x, centres, variance);   % gaussian kernel basis function    
    end  
    BX              = model.phi(X) ;                                    % K(X)
    ...
    for alpha_id = 1:model.dim_r        
        ...
        if (model.nmse > optimal.nmse) && (model.nmse > 1e-8)
            break ; 
        else         
            optimal   = model ;  
        end             
    ...
    optimal.A       =  @(q) predict_A (optimal, q, Iu) ;  % a function to predict the projection matrix
    fprintf('\t Found %d constraint vectors with residual error = %4.2e\n', optimal.dim_k, optimal.nmse) ;
end
function H = phi_gaussian_rbf_GMM(X,model)
    dim_N = size(X,2);
    for t = 1:dim_N
        for i=1:model.nbStates
            H(i,t) = gaussPDF(X(:,t), model.Mu(:,i), model.Sigma(:,:,i));
        end
...
function A = predict_A (model, q, Iu)
    Rn = Iu ;                               % initial rotation
    A  = zeros(model.dim_k, model.dim_u) ;  % Initial constraint matrix
    bx = model.phi(q) ;                     % gaussian kernel of q    
    for k = 1:model.dim_k
...
function model = search_alpha (BX, RnUn, model)
    ...
    model.nmse          = 100000000 ; 
    for i = 1:2 % normally, the 1 attempt is enough to find the solution. Repeat the process if the process tends to find local minimum (i.e., for i = 1:5) 
