...
function ROBOT=dlr_7dof()
    ...
    L{2} = Link([-pi/2 0 0 0.3  0],'standard');
    L{3} = Link([-pi/2 0 0 0.4  0],'standard');
    L{4} = Link([ pi/2 0 0 0.5  0],'standard');
    L{5} = Link([ pi/2 0 0 0.39 0],'standard');
    L{6} = Link([-pi/2 0 0 0    0],'standard');
    L{7} = Link([ 0    0 0 0.2  0],'standard');
    ROBOT = SerialLink([L{1},L{2},L{3},L{4},L{5},L{6},L{7}]);
end      

function ns_policy = policy_linear(x,settings_null)
    w = eye(7);
    ns_policy = -w*(x-settings_null.target);
end